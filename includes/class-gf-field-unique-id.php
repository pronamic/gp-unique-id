...
class GF_Field_Unique_ID extends GF_Field {
	...
	public function handle_sequential_edits( $value, $entry, $field, $form, $input_id ) {
		// Note: We cannot check for field type here as GravityView changes our "uid" field type to "text" when editing an entry.
		if ( self::$_is_gpuid_generated || $field->{'gp-unique-id_type'} !== 'sequential' || empty( $value ) || $value == rgar( $entry, $field->id ) ) {
			return $value;
		}

		$atts   = gp_unique_id()->get_field_unique_id_attributes( $field, $form, $entry );
		$prefix = GFCommon::replace_variables( $atts['prefix'], $form, $entry, false, true, false, 'text' );
		$suffix = GFCommon::replace_variables( $atts['prefix'], $form, $entry, false, true, false, 'text' );
